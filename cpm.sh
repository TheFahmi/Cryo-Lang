...
cmd_build() {
    ...
    if command -v docker &> /dev/null; then
        ...
        docker run --rm -v "${mount_path}:/src" -w //src cryo-toolchain bash -c "
            cryoc ${target} && \
            clang++ -O2 -Wno-override-module ${target}.ll /usr/lib/libruntime_cryo.a -o ${target%.cryo}.out -lpthread -ldl
        "
        
        if [[ -f "${target%.cryo}.out" ]]; then
            print_success "Built: ${target%.cryo}.out"
        else
